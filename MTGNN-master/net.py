...
class gtnet(nn.Module):
    def __init__(self,batch_size, horizon,cycle,gcn_true, buildA_true, gcn_depth, num_nodes, device, predefined_A=None, static_feat=None, dropout=0.3, subgraph_size=20, node_dim=40, dilation_exponential=1, conv_channels=32, residual_channels=32, skip_channels=64, end_channels=128, seq_length=12, in_dim=2, out_dim=12, layers=3, propalpha=0.05, tanhalpha=3, layer_norm_affline=True):
        ...
        self.decay_rate2 = nn.Parameter(torch.tensor(1.0), requires_grad=True)
    ...
    def forward(self, input, idx=None):
        ...
        # 周期衰减
        # 创建一个与 final_adp 形状相同的张量来表示时间戳
        timestamps = torch.arange(1, final_adp.shape[1] + 1).unsqueeze(0).unsqueeze(-1).unsqueeze(-1).to(self.device)  # 1*5*1*1
        # 计算衰减因子矩阵
        decay_factor2 = torch.exp(-self.decay_rate2 * (final_adp.shape[1] - timestamps))
        # 将衰减因子与 final_adp1 进行元素级别的乘法,得到衰减后的距离矩阵
        final_adp_decay = final_adp * decay_factor2
        final_adp1 = torch.sum(final_adp_decay, dim=1, keepdim=True) / (torch.sum(decay_factor2, dim=1, keepdim=True))  # 4*1*8*8
